...
const submit = async () => {
    ...
    const { data, error } = await supabase.from('kelas').update({
        nama: nama.value,
    }).eq('id', $route.params.id);
    ...
    if (error) {
        ...
        navigateTo("/admin/datamaster/kelas");
    }
};
  ...
  <NuxtLayout name="authenticated">
    ...
    <SectionMain>
      ...
      <div class="bg-blue-100 bordeconst kelas = ref('');r-t border-b border-blue-500 text-blue-700 px-4 py-3" role="alert" v-if="false"> 
        <p class="font-bold">Tambah kelas</p>
        <p class="text-sm">Berhasil Menambahkan kelas</p>
      </div>
      <CardBox>
        ...
        <form @submit.prevent="submit">
          ...
          <FormField label="nama">
            <FormControl v-model="nama" placeholder="Masukan Nama kelas" :icon="mdiAccount" />
